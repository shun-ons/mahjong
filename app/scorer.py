...
class MahjongScorer:
    def __init__(self, hand: list[str], melds: list[Meld], agari_hai: str, **game_state):
        """
        MahjongScorerの初期化.
    ...
    def calculate(self) -> dict:
        ...
        """
        # 手牌の解析を行う.
        analysis = HandAnalysis(self.hand, self.melds, self.agari_hai).agari_combinations
        if not analysis:
            return {"error": "アガリ形ではありません。"}
        # 高点法で最高点を選ぶ (簡単のため、最初の分析結果を使用)
        best_analysis = analysis[0]
        # 役を計算.
        found_yaku, han = YakuChecker.check_all_yaku(best_analysis, self.melds, self.game_state)
        if han == 0:
            return {"error": "役がありません。"}
        # 符を計算.
        fu = FuCalculator.calculate_fu(best_analysis, self.melds, found_yaku, self.game_state)
        # ドラを計算.
        dora_han = self._count_dora()
        total_han = han + dora_han
        # 最終的な点数を計算して報告.
        score, name = self._get_final_score(total_han, fu)
        return {"yaku": found_yaku, "han": total_han, "fu": fu, "score_name": name, "score": score}
    ...
    def _count_dora(self) -> int:
        ...
        uradora = [Tile.next_tile(t) for t in ura_dora_indicators if t] if is_reach else []
        # 全ての手牌を一つのリストにまとめる.
